#include "ircbloq_tcs3200.h"

Ircbloq_tcs3200::Ircbloq_tcs3200(uint8_t s2, uint8_t s3, uint8_t s0, uint8_t s1, uint8_t oe)
{
    device = new MD_TCS230(s2, s3, s0, s1, oe);
}

void Ircbloq_tcs3200::begin()
{
    device->begin();
}

void Ircbloq_tcs3200::calibrateWhite()
{
    device->read();
...
void Ircbloq_tcs3200::calibrateBlack()
{
    device->read();
    while(!device->available())
        ;
...
void Ircbloq_tcs3200::mesureColor()
{
    device->read();
    while(!device->available())
        ;
...
uint32_t Ircbloq_tcs3200::getColorValue(uint8_t color)
{
